...
while (~done)
  ...
  if (largescale)
    ...
    H11p = A'*(  diag( sparse(sigx) )*A  );
    opts.POSDEF = true; opts.SYM = true;
    % add full to make H11p is not sparse, added by Zhijun 2011-02-26
     %[dx, hcond] = linsolve(full(H11p), w1p,opts);
  ...
  %disp(sprintf('Iteration = %d, tau = %8.3e, Primal = %8.3e, PDGap = %8.3e, Dual res = %8.3e',pditer, tau, sum(u), sdg, norm(rdual)));
  if (largescale)
