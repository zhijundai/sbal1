function [U,P,ress] = bundle(U,P,u,iter);
...
% 2011 Zhijun Dai (dai.zhijun@gmail.com), the original code are modified to get the residual in each
% iteration.
if size(U,1) == 3
U = [U; ones(1,size(U,2))];
end
lambda = 0.01; 
fprintf('Iter:\t Error L2:\t Error L1:\t lambda:\n');
...
for i = 1:iter
    ...
    res_l1 = compute_res_l1(Pnew,Unew,u);
    if abs(resnew - res) < 1e-6
        break;
    end
    while resnew > res && lambda < 1e6
        lambda = lambda*2;
        C = (A'*A+lambda*speye(size(A,2),size(A,2)));
        c = A'*B;
        d = -C\c;
    ...
    U = Unew;
    P = Pnew;
    fprintf('%d\t%f\t%f\t%f\n',i,resnew,res_l1,lambda);
    ress=[ress [res_l1; resnew]];
end
U = U(1:3,:);
